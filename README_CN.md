...
- 🎨 **极致平滑的折叠 UI** — 底部配置触发器，采用顶部锚定的平滑下拉动画，保持核心信息区的绝对视觉稳定。默认英语，主题颜色自动跟随系统，支持手动切换。
- ⌨️ **无界面的极速剪切板流转** — 告别繁琐的输入框！开启快捷键后，直接使用 `Ctrl + Shift + 1~9` 精准提取从最新到更早的剪切板文件历史，快如闪电。配有独立的全局开关，把控制权完全交还给用户。
...
- [x] 支持多语言切换与系统主题自动跟随
- [x] 底部触发器 + 顶部锚定下拉动画
- [x] 无输入框纯快捷键剪切板流转 (Ctrl+Shift+1~9)
- [x] 快捷键全局开关 + localStorage 持久化
- [ ] 核心：剪切板智能解析功能（功能逻辑）

...
- 🎨 **极简折叠 UI 与系统级适配** — 引入汉堡菜单，核心界面极致清爽。默认英语，主题颜色自动跟随系统，支持手动切换。
- 📋 **高级剪切板流转** — 支持解析历史剪切板内容，可指定顺位编号读取。
- ⌨️ **自定义快捷键控制** — 采用不冲突的 `Ctrl + Shift + 数字` 快速提取对应顺位的剪切板记录，并配有独立的快捷键全局开关，把控制权完全交还给用户。
